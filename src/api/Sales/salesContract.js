...
const salesContractAPI = {
  ...
  // GET /api/dhinesh/getSalesContractDocId
  getSalesContractDocId: async ({ financialYear, orgId, screenCode }) => {
    try {
      ...
      const response = await apiClient.get(
        `/api/dhinesh/getSalesContractDocId?${params.toString()}`,
      );

      return response?.paramObjectsMap?.invoiceDocId || "";
    } catch (error) {
      console.error("Error fetching sales contract doc id:", error);
      throw error;
    }
  },
};
