...
const quotationAPI = {
  ...
  // Get quotation document ID
  getQuotationDocId: async ({ financialYear, orgId, screenCode }) => {
    try {
      ...
      const response = await apiClient.get(
        `/api/quotationservice/getQuotationDocId?${params.toString()}`,
      );

      return response?.paramObjectsMap?.invoiceDocId || "";
    } catch (error) {
      console.error("Error fetching quotation doc id:", error);
      throw error;
    }
  },
};
