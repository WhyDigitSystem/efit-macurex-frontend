...
const ScreenNamesList = ({ onAddNew, onEdit, onBack }) => {
    ...
    const loadScreens = async () => {
        try {
            setLoading(true);
            setError(null);
            const response = await screensAPI.getScreens(ORG_ID);

            // Handle different response formats
            if (Array.isArray(response)) {
                setList(response);
            } else if (response?.paramObjectsMap?.screenNamesVO) {
                setList(response.paramObjectsMap.screenNamesVO);
    ...
    // Filter screens based on search with safe access
    const filtered = list.filter((screen) => {
        ...
        <div className="max-w-6xl mx-auto bg-gray-50 dark:bg-gray-900 p-4">
            ...
            <div className="flex justify-between items-center mb-4">
                <div className="flex items-center gap-3">
                    ...
                    >
                        ...
                        <span className="text-lg font-semibold">Screen Names</span>
                    </button>
                </div>
          ...
          rounded-md text-xs hover:bg-purple-700 transition disabled:opacity-50"
            ...
            <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 flex items-center gap-2 mb-4 shadow-sm">
                ...
                <input
                    type="text"
                    placeholder="Search screens by name or code…"
            ...
            <div className="flex justify-between items-center mb-3">
                ...
                {!loading && totalItems > 0 && (
                    <div className="text-xs text-gray-600 dark:text-gray-400">
                        Showing {startIndex + 1}-{endIndex} of {totalItems} screens
                        {search && ` for "${search}"`}
                    </div>
                )}
                ...
                <div className="rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700 shadow-sm mb-4">
                    <table className="w-full text-sm">
                        ...
                        <thead>
                            <tr className="bg-gray-100 dark:bg-gray-800 text-xs text-gray-600 dark:text-gray-300">
                                ...
                                <th className="p-2 text-left font-medium">Screen Code</th>
                                <th className="p-2 text-left font-medium">Screen Name</th>
                  ...
                  hover:bg-gray-50 dark:hover:bg-gray-800/50 transition"
                                >
                                    ...
                                    <td className="p-2">{screen.screenCode || "-"}</td>
                                    <td className="p-2">{screen.screenName || "-"}</td>
                        ...
                        ${screen.active
                            ...
                            {currentItems.length === 0 && !loading && !error && (
                                <tr>
                                    ...
                                    >
                                        {search ? "No screens found matching your search" : "No screens available"}
                                    </td>
                                </tr>
                            )}
                        </tbody>
