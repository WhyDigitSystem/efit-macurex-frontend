...
// import { masterAPI } from "../../../api/unitConversionAPI";

const UPPERCASE_FIELDS = ["conversionCode"];

/* ---------------------------------------------------------------------------
   ONE shared, compact control style, used for text/number inputs and the
   checkbox row alike, so every field in the form has the identical height,
...
const UnitConversionMasterForm = ({ data, onBack }) => {
  ...
  const validate = () => {
    ...
    if (!form.conversionCode.trim())
      errors.conversionCode = "Conversion Code is required";

    if (!form.fromUnit.trim())
      errors.fromUnit = "From Unit is required";

    if (!form.toUnit.trim())
      errors.toUnit = "To Unit is required";

    if (
      form.fromUnit.trim() &&
      form.toUnit.trim() &&
      form.fromUnit.trim().toLowerCase() === form.toUnit.trim().toLowerCase()
    )
      errors.toUnit = "To Unit must be different from From Unit";
  ...
  const handleSave = async () => {
    ...
    try {
      // await masterAPI.saveUnitConversion(payload);
      ...
      console.error(error);
      alert("Failed to save Unit Conversion.");
    } finally {
    ...
    <div className="p-2 max-w-7xl">
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <h2 className="text-base font-semibold text-gray-900 dark:text-white">
          {data ? "Edit Unit Conversion" : "Add Unit Conversion"}
        </h2>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-3 space-y-3">
        ...
        {/* Conversion Details */}
        <div>
          <SectionHeader>Conversion Details</SectionHeader>

          <div className={fieldGrid}>
            <Field
              label="Conversion Code"
              name="conversionCode"
              value={form.conversionCode}
              onChange={handleChange}
              error={fieldErrors.conversionCode}
              required
            />
