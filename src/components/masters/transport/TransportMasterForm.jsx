...
const TransportMasterForm = ({ onBack, onSave, editData }) => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "transportName") {
      ...
      if (value.length > 100) {
        setFieldErrors((prev) => ({
          ...prev,
          transportName: "Transport Name must be maximum 100 characters",
        }));
        return;
      }
    }

    if (name === "address" && value.length > 250) {
      setFieldErrors((prev) => ({
        ...prev,
        address: "Address must be maximum 250 characters",
      }));
      return;
    }
  ...
  const handleSave = async () => {
    ...
    console.log(" Saving Transport Payload:", payload);

    try {
      const response = await transportAPI.updateCreateTransport(payload);
    ...
    <div className="p-2 max-w-7xl ">
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <h2 className="text-base font-semibold text-gray-900 dark:text-white">
          {editData ? "Edit Transport" : "Add Transport"}
        </h2>
      </div>
...
export default TransportMasterForm;
