...
const ExchangeRateForm = ({ onBack, onSave, editData }) => {
  ...
  const fetchCurrencies = async () => {
    try {
      ...
      const response = await exchangeRateAPI.getCurrencies(ORG_ID);
  ...
  // Currency Symbol drives Currency Name from the same currency master record
  const handleCurrencySymbolChange = (e) => {
    const currencySymbol = e.target.value;
    const selected = currencies.find(
      (c) => c.currencySymbol === currencySymbol,
    );

    if (fieldErrors.currencySymbol) {
      setFieldErrors((prev) => ({ ...prev, currencySymbol: "" }));
    }
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await exchangeRateAPI.updateCreateExchangeRate(payload);
    ...
    <div className="p-2 max-w-7xl ">
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <h2 className="text-base font-semibold text-gray-900 dark:text-white">
          {editData ? "Edit Exchange Rate" : "Exchange Rate"}
        </h2>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          <div>
            ...
            <select
              ...
              className={`${controlClasses} ${
                fieldErrors.currencySymbol ? "border-red-500" : ""
              }`}
            >
          ...
          {/* Currency Name - auto filled from Currency Symbol, read-only */}
          <div>
            ...
            <input
              ...
              disabled
              className={`${controlClasses} bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 cursor-not-allowed`}
            />
          </div>
