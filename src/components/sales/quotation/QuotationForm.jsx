...
const QuotationForm = ({ data, onBack }) => {
    ...
    const loadQuotationData = async (quotationId) => {
        ...
        try {
            ...
            if (response?.status === true && response?.paramObjectsMap?.quotationResponseVO) {
                ...
                setValue("date", quotation.docDate || new Date().toISOString().split('T')[0]);
                ...
                // Set quotation items
                if (quotation.quotationItemDetailsResponseDTO && quotation.quotationItemDetailsResponseDTO.length > 0) {
                    const items = quotation.quotationItemDetailsResponseDTO.map(item => ({
                ...
                // Set tax details
                if (quotation.quotationItemTaxDetailsDTO && quotation.quotationItemTaxDetailsDTO.length > 0) {
                ...
                if (quotation.quotationIemFileUploadDetailsDTO && quotation.quotationIemFileUploadDetailsDTO.length > 0) {
                    const attachments = quotation.quotationIemFileUploadDetailsDTO.map(file => ({
                        attachment: {
                ...
                addToast(response?.paramObjectsMap?.message || "Failed to load quotation data", "error");
            }
        } catch (error) {
    ...
    const onSubmit = async (formData) => {
        ...
        try {
            ...
            const quotationData = {
                ...
                customerEnquiryDate: formatDateForAPI(formData.customerEnquiryDate) || "",
                ...
                quotationItemDetailsDTO: (formData.quotationItems || []).map(item => ({
                    basicPrice: Number(item.basicPrice || 0),
                    currencyName: Number(item.currencyName) || 0,
                    deliveryDate: formatDateForAPI(item.date) || new Date().toISOString().split('T')[0],
                ...
                date: formatDateForAPI(formData.date) || new Date().toISOString().split('T')[0],
                amount: Number(grossAmount || 0),
                totalAmount: Number(grossAmount || 0) + Number(formData.chargesSummary?.freight || 0),
            };
            ...
            formDataToSend.append("quotation", quotationDataBlob, "quotationDTO.json");
    ...
    if (loading) {
        return (
            <div className="flex items-center justify-center h-64">
                <div className="text-gray-500 dark:text-gray-400">Loading quotation data...</div>
            </div>
        );
    }
                                        ...
                                        <InputCell
                                            ...
                                            onChange={(e) => {
                                                ...
                                                const total = rows.reduce((sum, row, i) => {
                                                    ...
                                                    return String(row.particulars) === String(freightOption?.value)
                                                        ? sum + amount
                                                        : sum;
                                                }, 0);
