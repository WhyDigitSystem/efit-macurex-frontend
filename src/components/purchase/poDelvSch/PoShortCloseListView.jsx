...
const PoShortCloseListView = ({ onAddNew, onEdit, onBack }) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const response =
        await poDelScheduleAPI.getPurchaseOrderDeliveryScheduleShortCloseByOrgId(
          BRANCH_ID,
          ORG_ID,
        );
      ...
      if (
        response?.status &&
        response?.paramObjectsMap?.purchaseOrderDeliveryScheduleShortCloseVO
      ) {
        records =
          response.paramObjectsMap.purchaseOrderDeliveryScheduleShortCloseVO;
      ...
      const transformedData = records.map((item) => ({
        ...
        totalPendingQty:
          item.purchaseOrderDeliveryScheduleShortCloseDetailsResponseDTO
            ?.reduce((total, detail) => total + (detail.pendingQty || 0), 0)
            .toFixed(3) || "0.000",
