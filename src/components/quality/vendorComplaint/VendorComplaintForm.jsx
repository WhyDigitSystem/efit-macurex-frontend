...
const CHILD_TABS = [
  { key: "complaintDetail", label: "Complaint Detail", kind: "table" },
...
const VendorComplaintForm = ({ data, onBack }) => {
  const { addToast } = useToast();
  const orgId = Number(localStorage.getItem("orgId")) || 0;
  const branch = Number(localStorage.getItem("branchId")) || 0;
  const usersId = localStorage.getItem("usersId");

  const [activeChildTab, setActiveChildTab] = useState("complaintDetail");
  ...
  const [detailRows, setDetailRows] = useState(() => {
    ...
    return [{ partNo: "", partName: "", qty: "", reason: "" }];
  });
  ...
  const validate = () => {
    ...
    if (!header.fgItem) errors.fgItem = "FG Item is required";
    if (!header.docNo?.trim()) errors.docNo = "Doc No is required";
    if (!header.fgName?.trim()) errors.fgName = "FG Name is required";
    if (!header.docDate) errors.docDate = "Doc Date is required";
    if (!header.supplierId) errors.supplierId = "Supplier No is required";
  ...
  const handleSave = async () => {
    ...
    try {
      const response =
        await vendorComplaintAPI.createUpdateVendorComplaint(payload);

      if (response?.status) {
        addToast(
          response?.paramObjectsMap?.message ||
            (isUpdate
              ? "Vendor Complaint updated successfully!"
              : "Vendor Complaint created successfully!"),
        );
        onBack?.();
      } else {
        addToast(
          response?.errors?.[0]?.shortMessage ||
            ...
            response?.message ||
            response?.paramObjectsMap?.message ||
            "Failed to save Vendor Complaint.",
        );
      }
    } catch (err) {
      console.error("Save Vendor Complaint Error:", err);
    ...
    <div className="w-full p-2">
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <h2 className="text-base font-semibold text-gray-900 dark:text-white">
          {data ? "Edit Vendor Complaint" : "Add Vendor Complaint"}
        </h2>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-3 space-y-4">
        ...
        <section className="mt-0 bg-white dark:bg-gray-800">
          ...
          {/* Tab 1: Complaint Detail */}
          {activeChildTab === "complaintDetail" && (
            <div className="pt-3">
              <DynamicTable
                columns={[
                  {
...
export default VendorComplaintForm;