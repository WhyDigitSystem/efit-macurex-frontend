...
import { profileAPI } from "../../api/profileApi"; // adjust path to wherever profileAPI.js lives
import {
  Save,
  X,
  Shield,
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      try {
        ...
        const userData = res?.paramObjectsMap?.userVO;

        setProfile({
          name: userData?.employeeName || userData?.userName,
          email: userData?.email,
          username: userData?.userName,
          type: userData?.userType,
          role: userData?.role,
          mobile: userData?.mobileNo,
          company: userData?.companyVO?.companyName,
          status:
            userData?.active === "Active" || userData?.loginStatus
              ? "Active"
              : "Inactive",
        });
      } catch (err) {
            ...
            <div className="flex justify-center mb-4">
              <div
                className={`p-3 rounded-full ${
                  successMessage ? "bg-emerald-500/10" : "bg-rose-500/10"
                }`}
              >
                {successMessage ? (
                  <Check
          ...
          <nav className="mt-4 flex lg:flex-col gap-2">
            ...
            >
              <Shield
                size={16}
                className={
                  activeTab === "security" ? "text-[var(--accent)]" : ""
                }
              />
              Security
            </button>
