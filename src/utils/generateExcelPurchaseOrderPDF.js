...
/**
 * Generate a professional Excel Purchase Order PDF — project themed.
 * Layout/logic mirrors generateQuotationPDF.js / generatePurchaseContractPDF.js /
 * generatePurchaseIndentPDF.js exactly.
 *
 * @param {Object} data
 * @param {Object}   data.company   - { name }
 * @param {Object}   data.header    - { plantId, department, belongsTo, supplierCode, supplierName,
 *                                      taxCode, gstState, isIgstApplicable, gstnNo, refNo, refDate,
 *                                      poNo, poDate, address }
 * @param {Array}    data.items     - [{ itemDescription, hsnSacCode, taxType, taxPercent, qty,
 *                                      purchaseUnit, rate, amount, sgstRate, sgstAmount, cgstRate,
 *                                      cgstAmount, igstRate, igstAmount }]
 * @param {Array}    data.taxDetails- [{ particulars, taxPercent, taxAmount }]
 * @param {Object}   data.terms     - { discount, paymentTerms, totalAmount, deliveryTerms, freight,
 *                                      freightType, packingType, insurance, modeOfDespatch,
 *                                      inlandCharge, preparedBy, authorizedBy, narration }
 */
export function generateExcelPurchaseOrderPDF(data) {
  ...
  const checkPageBreak = (needed) => {
    if (y + needed > PAGE_H - M - 30) {
      ...
      doc.text(
        `${company.name || "Company Name"} — PURCHASE ORDER`,
        PAGE_W / 2,
        y + 15,
        { align: "center" },
      );
  ...
  /* ================================================================ */
  /* 2. HEADER BLOCK — company (left) | PO details (right)             */
  /* ================================================================ */
  const leftRows = [
    ["Plant ID :", header.plantId],
    ["Department :", header.department],
    ["Belongs To :", header.belongsTo],
    ["Supplier ID :", header.supplierCode],
    ["Supplier Name :", header.supplierName],
    ["Tax Code :", header.taxCode],
    ["GST State :", header.gstState],
    ["Address :", header.address],
  ];

  const rightRows = [
    ["P.O. No :", header.poNo],
    ["P.O. Date :", header.poDate],
    ["Ref No :", header.refNo],
    ["Ref Date :", header.refDate],
    ["Is IGST Appl :", header.isIgstApplicable ? "Yes" : "No"],
    ["GSTN No :", header.gstnNo],
  ];
  ...
  if (taxDetails.length > 0) {
    ...
    const taxRows = taxDetails.map((tax, idx) => [
      ...
      tax.taxAmount != null ? fmt(tax.taxAmount) : "",
    ]);

    const taxTableResult = autoTable(doc, {
      startY: y,
      margin: { left: M, right: M },
      head: [["S.No", "Particulars", "Tax %", "Tax Amount"]],
      body: taxRows,
      theme: "grid",
      styles: {
        fontSize: 8,
  ...
  const allPairs = [
    ["Discount", terms.discount != null ? fmt(terms.discount) : ""],
    ["Payment Terms", terms.paymentTerms],
    ["Total Amount", terms.totalAmount != null ? fmt(terms.totalAmount) : ""],
    ["Delivery Terms", terms.deliveryTerms],
    ["Freight", terms.freight != null ? fmt(terms.freight) : ""],
    ["Freight Type", terms.freightType],
    ["Packing Type", terms.packingType],
    ["Insurance", terms.insurance],
    ["Mode of Despatch", terms.modeOfDespatch],
    [
      "Inland Charge",
      terms.inlandCharge != null ? fmt(terms.inlandCharge) : "",
    ],
    ["Prepared By", terms.preparedBy],
    ["Authorized By", terms.authorizedBy],
  ...
  /* OUTPUT                                                            */
  /* ================================================================ */
  const fileName = `ExcelPurchaseOrder_${header.poNo || "document"}.pdf`;
...
export default generateExcelPurchaseOrderPDF;
