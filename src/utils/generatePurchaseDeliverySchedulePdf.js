...
/* ================================================================ */
/**
 * @param {Object} record - a row from PurchaseDeliveryScheduleList's data
 *                          (has `scheduleDetails`, each optionally with a
 *                          nested `schedule` array of { planDate, weekNo, scheduleQty })
 */
export async function generatePurchaseDeliverySchedulePdf(record) {
  ...
  const checkPageBreak = (needed) => {
    if (y + needed > PAGE_H - M - 30) {
      ...
      doc.text(
        `${company.companyName || "Company Name"} — PURCHASE DELIVERY SCHEDULE`,
        PAGE_W / 2,
        y + 15,
        { align: "center" },
      );
  ...
  doc.setTextColor(...C.primary);
  doc.text("PURCHASE DELIVERY SCHEDULE", PAGE_W / 2, y + 26, {
    align: "center",
  });
  doc.setTextColor(...C.black);
  y += 40;
  ...
  /* ================================================================ */
  /* 2. HEADER BLOCK — company/supplier (left) | doc details (right)   */
  /* ================================================================ */
  const HEADER_H = 150;
  doc.setDrawColor(...C.primary);
  doc.setLineWidth(0.8);
  ...
  leftRow("Supplier Code :", record.supplierCode);
  leftRow("Supplier Name :", record.supplierName);
  ...
  rightRow("Doc Date :", record.docDate || "");
  rightRow("Sch. Start Date :", record.schStartDate || "");
  rightRow("Sch. End Date :", record.schEndDate || "");
  rightRow("PO No :", record.poNo || "");
  rightRow("PO Date :", record.poDate || "");
  rightRow("Status :", record.active || "");

  doc.setTextColor(...C.black);
  y += HEADER_H;

  /* ================================================================ */
  /* 3. SCHEDULE DETAILS TABLE                                         */
  /* ================================================================ */
  sectionHeading("Schedule Details");

  const details = record.scheduleDetails || [];

  const itemRows = details.map((d, idx) => [
    idx + 1,
    d.itemCode || d.item?.itemCode || "",
    d.primaryUnit || "",
    d.purchaseUnit || "",
    fmtAmount(d.demandQty),
    fmtAmount(d.availableStockQty ?? d.availableStock),
    fmtAmount(d.qty),
    fmtAmount(d.tentativeQty),
    fmtAmount(d.tentativeQtyNextMonth),
    fmtAmount(d.rate),
  ]);
  ...
  /* ================================================================ */
  /* 4. WEEKLY SCHEDULE BREAKDOWN (per item, if provided)              */
  /* ================================================================ */
  const scheduleRows = [];
  details.forEach((d) => {
    (d.schedule || []).forEach((s) => {
      scheduleRows.push([
        ...
        s.planDate || "",
        s.weekNo ?? "",
        fmtAmount(s.scheduleQty),
      ]);
    });
  });

  if (scheduleRows.length > 0) {
    sectionHeading("Weekly Schedule Breakdown");

    const scheduleTableResult = autoTable(doc, {
      startY: y,
      margin: { left: M, right: M },
      head: [["Item Code", "Plan Date", "Week No", "Schedule Qty"]],
      body: scheduleRows,
      theme: "grid",
      styles: {
        fontSize: 7,
  ...
  const allPairs = [
    ["Prepared By", record.preparedByName || record.preparedBy || ""],
    ["Note", record.note || ""],
  ];
  ...
  /* ================================================================ */
  const fileName = `PurchaseDeliverySchedule_${(record.docNo || "document").replace(/\//g, "-")}.pdf`;
...
export default generatePurchaseDeliverySchedulePdf;
