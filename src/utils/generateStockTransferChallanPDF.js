...
export function generateStockTransferChallanPDF(data) {
  ...
  const isIGST  = inv.isIgstApplicable === "Yes" || inv.isIgstApplicable === true;
  ...
  const checkPageBreak = (needed) => {
    if (y + needed > PAGE_H - M - 30) {
      ...
      doc.text(`Stock Transfer Challan — ${inv.docId || ""}`, PAGE_W / 2, y + 15, { align: "center" });
  ...
  doc.setTextColor(...C.primary);
  doc.text("STOCK TRANSFER CHALLAN", PAGE_W / 2, y + 26, { align: "center" });
  doc.setTextColor(...C.black);
  y += 40;
  ...
  leftRow("Location :", inv.locationName || inv.locationId);
  leftRow("GSTIN No :", inv.gstinNo);
  ...
  rightRow("Doc ID :", inv.docId);
  rightRow("Transfer Date :", inv.transferDate);
  rightRow("Type :", inv.type);
  rightRow("Time of Transfer :", inv.timeOfTransfer);
  ...
  /* ================================================================ */
  /* 3. SECONDARY HEADER ROW — packages, IGST, import/local, etc.     */
  /* ================================================================ */
  checkPageBreak(28);
  doc.setFillColor(...C.white);
  doc.setDrawColor(...C.primary);
  ...
  secField("Party GST State:", inv.partyGstState);
  secField("IGST Applicable:", inv.isIgstApplicable);
  secField("Import/Local:", inv.importLocal);
  secField("No. of Packages:", inv.noOfPackages);
  ...
  const baseHeaders = ["S.No", "Item Code", "Description", "HSN/SAC", "Tax Type", "Tax %", "Unit", "Stock", "Qty", "Rate", "Assessable Value"];
  let taxHeaders;
  if (isIGST) {
  ...
  const itemRows = items.map((item, idx) => {
    const row = [
      ...
      item.totalAssessableValue != null ? fmt(item.totalAssessableValue) : "",
    ];
    if (isIGST) {
      row.push(
  ...
  if (taxDtls.length > 0) {
    ...
    const taxResult = autoTable(doc, {
      startY: y,
      margin: { left: M, right: M },
      head: [["S.No", "Particulars", "Accepted Qty Amount", "Revised Amount"]],
      body: taxRows,
      theme: "grid",
      ...
      columnStyles: { 0: { cellWidth: 25, halign: "center" }, 2: { halign: "right" }, 3: { halign: "right" } },
      didDrawPage: (eventData) => { y = eventData.cursor.y; },
    });
  ...
  const fileName = `Stock_Transfer_Challan_${inv.docId || "document"}.pdf`;
...
export default generateStockTransferChallanPDF;
