...
/* ================================================================ */
/**
 * @param {Object} record - a row from PurchaseOrderAmendmentList's data
 *                          (header fields + poDetails array)
 */
export async function generatePurchaseOrderAmendmentPdf(record) {
  ...
  const checkPageBreak = (needed) => {
    if (y + needed > PAGE_H - M - 30) {
      ...
      doc.text(
        `${company.companyName || "Company Name"} — PURCHASE ORDER AMENDMENT`,
        PAGE_W / 2,
        y + 15,
        { align: "center" },
      );
  ...
  doc.setTextColor(...C.primary);
  doc.text("PURCHASE ORDER AMENDMENT", PAGE_W / 2, y + 26, { align: "center" });
  doc.setTextColor(...C.black);
  y += 40;
  ...
  leftRow("Party Code :", record.partyCode);
  leftRow("Party Name :", record.partyName);
  leftRow("Currency :", record.currency);
  ...
  /* ================================================================ */
  /* 3. PO DETAIL / REVISION TABLE                                     */
  /* ================================================================ */
  sectionHeading("PO Detail Revisions");

  const details = record.poDetails || record.poDetail || [];

  const itemRows = details.map((d, idx) => [
    idx + 1,
    d.itemCode || "",
    d.itemName || "",
    fmtAmount(d.originalQty),
    fmtAmount(d.amendmentQty),
    fmtAmount(d.originalRate),
    fmtAmount(d.amendmentRate),
    d.originalDeliveryDate || "",
    d.amendmentDeliveryDate || "",
  ]);
  ...
  const allPairs = [
    ["Exchange Rate", fmtAmount(record.exchangeRate)],
  ...
  /* ================================================================ */
  const fileName = `POAmendment_${(record.amendmentNo || "document").replace(/\//g, "-")}.pdf`;
...
export default generatePurchaseOrderAmendmentPdf;
