...
/* ================================================================ */
/* Main generator — same design & page-flow logic as the Purchase    */
/* Contract Amendment PDF.                                           */
/* ================================================================ */
/**
 * @param {Object} record - a row from SupplierRateContractAmendmentList's data
 *                          (header fields + supplierRateDetails array + details + summary)
 */
export async function generateSupplierRateContractAmendmentPDF(record) {
  ...
  const checkPageBreak = (needed) => {
    if (y + needed > PAGE_H - M - 30) {
      ...
      doc.text(
        `${company.companyName || "Company Name"} — SUPPLIER RATE CONTRACT AMENDMENT`,
        PAGE_W / 2,
        y + 15,
        { align: "center" },
      );
  ...
  doc.setTextColor(...C.primary);
  doc.text("SUPPLIER RATE CONTRACT AMENDMENT", PAGE_W / 2, y + 26, {
    align: "center",
  });
  doc.setTextColor(...C.black);
  y += 40;
  ...
  leftRow("Contract Date :", fmtDate(record.contractDate));

  /* ---- Right: amendment details ---- */
  let ry = y + 18;
  ...
  rightRow("Amendment Date :", fmtDate(record.amendmentDate));
  rightRow("Revision No :", record.revisionNo ?? "");
  rightRow("Valid From :", fmtDate(record.validFrom));
  rightRow("Valid To :", fmtDate(record.validTo));
  ...
  /* ================================================================ */
  /* 3. VALIDITY REVISION TABLE                                        */
  /* ================================================================ */
  sectionHeading("Contract Validity Revision");

  const validityTableResult = autoTable(doc, {
    startY: y,
    margin: { left: M, right: M },
    head: [["Original Valid From", "Original Valid To", "New Valid From", "New Valid To"]],
    body: [
      [
        fmtDate(record.validFrom),
        fmtDate(record.validTo),
        fmtDate(record.newValidFrom),
        fmtDate(record.newValidTo),
      ],
    ],
    theme: "grid",
    styles: {
  ...
  y =
    (validityTableResult && validityTableResult.finalY
      ? validityTableResult.finalY
      : y) + 12;

  /* ================================================================ */
  /* 4. RATE REVISION TABLE                                            */
  /* ================================================================ */
  sectionHeading("Rate Revision Details");

  const details = record.supplierRateDetails || record.supplierRateDetail || [];
  ...
  const detailMap = record.details || {};
  ...
  const allPairs = [
    ["Freight Type", detailMap.freightType || ""],
    ["Packing Type", detailMap.packingType || ""],
    ["Insurance Amount", fmtAmount(detailMap.insuranceAmount)],
    ["Mode of Despatch", detailMap.modeOfDespatch || ""],
    ["Tax Description", detailMap.taxDescription || ""],
    ["Prepared By", detailMap.preparedBy || ""],
    ["Authorised By", detailMap.authorizedBy || ""],
    ["Remarks", detailMap.remarks || ""],
    ["Summary Notes", summaryMap.summaryNotes || ""],
    ["Approval Status", summaryMap.approvalStatus || ""],
    ["Additional Comments", summaryMap.additionalComments || ""],
  ];
  ...
  /* ================================================================ */
  const fileName = `SupplierRateContractAmendment_${(record.amendmentNo || "document").replace(/\//g, "-")}.pdf`;
...
export default generateSupplierRateContractAmendmentPDF;
