...
export function generateProformaInvoicePDF(data) {
  ...
  const isIGST = inv.isIgstApplicable === "Yes";
  ...
  const checkPageBreak = (needed) => {
    if (y + needed > PAGE_H - M - 30) {
      ...
      doc.text(`${company.name || "Company Name"} — PROFORMA INVOICE`, PAGE_W / 2, y + 15, { align: "center" });
  ...
  doc.setTextColor(...C.primary);
  doc.text("PROFORMA INVOICE", PAGE_W / 2, y + 26, { align: "center" });
  doc.setTextColor(...C.black);
  y += 40;
  ...
  rightRow("Invoice No :", inv.invoiceNo);
  rightRow("Invoice Date :", inv.invoiceDate);
  rightRow("PO No :", inv.poNo);
  rightRow("PO Date :", inv.poDate);
  ...
  /* ================================================================ */
  /* 3. KIND ATTENTION BAR                                             */
  /* ================================================================ */
  if (inv.kindAttention || inv.designation) {
    checkPageBreak(20);
    doc.setFillColor(...C.white);
    doc.setDrawColor(...C.primary);
    doc.setLineWidth(0.5);
    ...
    setFont("helvetica", 8, "bold");
    doc.setTextColor(...C.gray900);
    const attn = `Kind Attention: ${inv.kindAttention || ""}${inv.designation ? " — " + inv.designation : ""}`;
    doc.text(attn, M + 6, y + 12);
    doc.setTextColor(...C.black);
    y += 22;
  }
  ...
  const baseHeaders = ["S.No", "Item Code", "Part No", "Description", "HSN", "Tax Type", "Tax %", "Qty", "Unit", "Rate", "Amount"];
  let taxHeaders;
  if (isIGST) {
  ...
  const itemRows = items.map((item, idx) => {
    const row = [
      ...
      item.dispatchQty != null ? String(item.dispatchQty) : "",
      item.unit || "",
      item.orderRate != null ? fmt(item.orderRate) : "",
  ...
  const allPairs = [
    ...
    ["No. of Packages", terms.noOfPkg || "—"],
    ["Package Type", terms.pkgType || "—"],
    ["Mode of Transport", terms.modeOfTransport || "—"],
    ["Rate of Duty", terms.rateOfDuty || "—"],
    ["Tariff No", terms.tariffNo || "—"],
    ["Basic Value", fmt(terms.basicValue)],
    ["Gross Amount", fmt(terms.grossAmount)],
    ["Amount in Words", amountInWords || "—"],
    ["Delivery To", terms.deliveryTo || "—"],
    ["Payment Terms", terms.paymentTerms || "—"],
    ["Payment %", terms.paymentPercentage || "—"],
  ...
  const fileName = `Proforma_Invoice_${inv.invoiceNo || "document"}.pdf`;
...
export default generateProformaInvoicePDF;
